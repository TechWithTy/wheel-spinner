...
const THEME_PRESETS = [
  { name: "Default", size: 240, spinUpMs: 200, spinDownMs: 1100, accentColor: "#6d28d9", textColor: "#ffffff" },
  { name: "Compact", size: 200, spinUpMs: 150, spinDownMs: 900, accentColor: "#0ea5e9", textColor: "#ffffff" },
  { name: "Bold", size: 280, spinUpMs: 220, spinDownMs: 1300, accentColor: "#ef4444", textColor: "#ffffff" },
  { name: "Slow", size: 260, spinUpMs: 400, spinDownMs: 2200, accentColor: "#22c55e", textColor: "#111827" },
];
...
const DemoWheel = () => {
  ...
  useEffect(() => {
    try {
      ...
      if (typeof saved.showResultModal === "boolean") setShowResultModal(saved.showResultModal);
      if (typeof saved.showWheelPopup === "boolean") setShowWheelPopup(saved.showWheelPopup);
  ...
  }, [showResultModal, showWheelPopup, autoSpin, size, spinUpMs, spinDownMs, accentColor, textColor, preset, prizes]);
  ...
  function addPrize() {
    ...
    setPrizes((prev) => [...prev, { id, label: "New Prize", weight: 1, color: accentColor }]);
  }
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <label className="flex items-center gap-2">
          ...
          />
          <Label className="text-sm">Show Result Modal</Label>
        </label>
        <label className="flex items-center gap-2">
          <Switch
          ...
          />
          <Label className="text-sm">Popup Wheel</Label>
        </label>
        <label className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <label className="flex items-center gap-2">
          <Label className="text-sm w-24">Preset</Label>
          <select
        ...
        <label className="flex items-center gap-2">
          <Label className="text-sm w-24">Spin up (ms)</Label>
          <Input
            type="number"
            value={spinUpMs}
            min={50}
        ...
        <label className="flex items-center gap-2">
          <Label className="text-sm w-24">Spin down (ms)</Label>
          <Input
            type="number"
            value={spinDownMs}
            min={200}
        ...
        <label className="flex items-center gap-2">
          ...
          <Input type="color" value={accentColor} onChange={(e) => setAccentColor(e.target.value)} className="h-8 w-16 p-1" />
        </label>
        <label className="flex items-center gap-2">
          <Label className="text-sm w-24">Text</Label>
          <Input type="color" value={textColor} onChange={(e) => setTextColor(e.target.value)} className="h-8 w-16 p-1" />
        </label>
      </div>
      ...
      <p className="text-xs text-muted-foreground">Note: Hourly cadence. Use admin reset for repeated testing.</p>
    </div>
  );
};
